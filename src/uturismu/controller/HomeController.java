...
public class HomeController {
	...
	public String showIndex(HttpSession session, Model model) {
		...
		if (isActiveSession(session)) {
			...
			} else if (isBooker(session)) {
				holidayResult = userService.getHolidayPackages();
			}
			List<HolidayPackageSummaryBean> holidayList = BeanMapping.encode(holidayResult);
		...
		List<HolidayPackageSummaryBean> list = BeanMapping.encode(userService.getHolidayPackages());
		model.addAttribute("holidayList", list);
		model.addAttribute("content", "holidayList.jsp");
		return "index";
	}
	...
	public String signup(@Valid TourOperatorSignup signup, BindingResult result, Model model,
		...
		if (result.hasErrors() || isSignupInvalid(signup)) {
	...
	private boolean isSignupInvalid(TourOperatorSignup signup) {
		if (signup.getCity() == 0)
			return true;
		return false;
	}
	...
	private boolean isSignupInvalid(BookerSignup signup) {
		if (signup.getGender().equals("-"))
			return true;
		if (signup.getBirthDay() == 0 || signup.getBirthMonth() == 0 || signup.getBirthYear() == 0)
			return true;
		if (signup.getBirthPlace() == 0)
			return true;
		if (signup.getCity() == 0)
			return true;
		return false;
