...
public class TourOperatorController {
	...
	@RequestMapping("/home")
	public String showHomePage(@RequestParam("id") Long id,Model model){
		
		System.out.println("TOUROPERATOR CONTROLLER");
		
		try{
			List<HolidayPackage> holiPack=touroperatorService.getAllHolidayPackages(id);
			List<Pack> packs=new LinkedList<Pack>();
			
			for (HolidayPackage holidayPackage : holiPack) {
				Pack pack=new Pack();
				pack.setId(holidayPackage.getId());
				pack.setName(holidayPackage.getName());
				pack.setDescription(holidayPackage.getDescription());
				pack.setPrize(getPackPrize(holidayPackage));
				packs.add(pack);
			}
